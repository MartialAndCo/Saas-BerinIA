    ...
    def _convert_apify_results_to_leads(self, apify_results, niche):
        ...
        for i, place in enumerate(apify_results):
            try:
                ...
                # Afficher le contenu complet du résultat pour débogage
                logger.info(f"Contenu complet du résultat Apify pour {company_name}: {json.dumps(place, ensure_ascii=False)}")
                
                # Rechercher explicitement des champs qui pourraient contenir l'email
                email_fields = ["email", "emailAddress", "contact_email", "contactEmail", "business_email", "businessEmail"]
                found_email = None
                for field in email_fields:
                    if field in place and place[field]:
                        found_email = place[field]
                        logger.info(f"Email trouvé dans le champ '{field}': {found_email}")
                        break
                ...
                lead = {
                    ...
                    "position": None,
                    "email": place.get("email") or place.get("emailAddress"),  # Récupérer l'email s'il existe
                    "phone": place.get("phone") or place.get("phoneNumber"),  # Le champ s'appelle 'phone' dans Apify
